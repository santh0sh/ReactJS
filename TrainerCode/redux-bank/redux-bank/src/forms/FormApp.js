  ...
  return <form onSubmit={handleSubmit} className="form">
    <div className="field">
      <div className="control">
        <Field name="firstName" component={renderField} id ="txt1" type="text" label="First Name"/>
      </div>
    </div>
...
const renderField = ({ input, label, type, meta: { touched, error, warning } }) => (
  <div>
    <div className="control">
      ...
      {touched && ((error && <span className="myerror">{error}</span>) || (warning && <span>{warning}</span>))}
    </div>
  </div>
)
...
class FormApp extends Component {
  ...
  handleSignIn = values => {
   ...
   alert("Sent data to database successfully")
  };

  render() {
    return (
      <div className="App">       
          <h1 className="bg-warning text-center">Welcome to React and  redux-form</h1>
